<?php session_start();
  ...
  require_once 'config/config.php';
	...
	if(isset($_GET['violator_id'])){
		...
		$sql = "select v.gender, v.pic, v.fname, v.mname, v.lname, v.address, v.bday, v.noted_by, vio.violation, p.offense,  vf.pen_id, vio.vio_id from violator as v left join violator_offense as vf on vf.v_id = v.v_id left join penalty as p on p.pen_id = vf.pen_id left join violation as vio on vio.vio_id = p.vio_id where v.v_id=?";
		$query = $theConnection->prepare($sql) or die(mysqli_error($theConnection));
		$query->bind_param('i', $violator_id);
		$query->execute();
		$query->bind_result($gender, $pic, $violator_fname, $violator_mname, $violator_lname, $address, $bday, $noted, $violation, $offense, $p_ids, $vio_id);
		while($query->fetch()) {
			$offenses[] = $offense;
								...
								<div style="margin-top:5px;" class="tab-content" style="color:black">
									<div id="first" class="active tab-pane">
										...
										<div class="form-group row">
											...
											<div class="col-lg-3">
											 	<input class="form-control" type="text" id="noted" value="<?php echo $noted; ?>" name="noted" />
											</div>
