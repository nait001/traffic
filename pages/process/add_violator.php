...
$query->bind_param('issssssss', $id, $fname, $mname, $lname, $address, $bday, $pic_name, $gender, $noted);
$error = "";
if($query->execute()){
	move_uploaded_file($pic_tmp, "../imgs/".$pic_name);
	$query->close();
	$query = $theConnection->query("select v_id from violator where fname = '$fname' and mname = '$mname' and lname = '$lname'") or die(mysqli_error($theConnection));
